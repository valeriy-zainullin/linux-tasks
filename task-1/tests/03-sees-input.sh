...
echo -n "$input" | xxd -p -r > /dev/pbchar
echo "Input was sent."

set -x

# In fact, write operation to /dev/pbchar won't return until
#   our driver pb_dev_read is not done with it's job.
# So we don't have to wait with "sleep" here.

dmesg | grep "phonebook got user_data: (Андрей, Александров, 25, +79999999999, andrejalexandrov@pushkapochta.ru)."
